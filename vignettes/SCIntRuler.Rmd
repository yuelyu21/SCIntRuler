...
# selected_cells <- sample(cell_names, size = 200)
...
```{r}
sim.list <- Seurat::SplitObject(sim_data, split.by = "Study")


fullcluster <- SCIntRuler::GetCluster(sim.list,50,200)

normCount <- SCIntRuler::NormData(sim.list)
...
```{r}
outres <- SCIntRuler::PermTest(fullcluster,distmat,15)
SCIntRuler::PlotSCIR(fullcluster,sim.list,outres,"")

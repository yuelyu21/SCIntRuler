...
FindCell <- function(seuratobj,seuratlist,fullcluster, distmat,firstn = 15){
  ...
  for (i in 1:length(seuratlist)){
    ...
    for (q in 1:nclust){
      ...
      disttest[[q]] <- data.frame(rbind(cbind(dist1,rep("Internal Dist",length(dist1))),
        ...
        mutate(dist = as.numeric(dist1),group = factor(V2)) %>% dplyr::select(3,4)

      testres[q,1] <- round(pvalue(coin::independence_test(dist ~ group, data = disttest[[q]],
  ...
  for (i in 1:length(fullcluster)){
    fullcluster1[[i]] <- fullcluster[[i]] %>%
      mutate(finecluster = factor(finecluster),
  ...
  meta <- meta %>% mutate(plot_cluster = dplyr::if_else(finecluster %in% plotc,finecluster, NA))

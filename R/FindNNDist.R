...
#' @param meaningn Positive integer giving the number of nearest-neighbor
  ...
  for (j in seq_along(fullcluster)) {
    ...
    for(q in seq_along(allCT)) {
      ...
      subc <- onecount[, sample(which(onemeta$finecluster == ct), min(20, max1)),
                       drop = FALSE] # dim 9763 * 10
      otherc <- onecount[, sample(which(onemeta$finecluster != ct), min(500, max2)),
                         drop = FALSE] # dim 9763 * 500
      # onecount 9763 x 2400 othercount 9763 * 3500

      # otherc_external 9763 * 1000
      otherc_external <- as.matrix(othercount[, sample(
        seq_len(ncol(othercount)),
        min(500 * length(normCount[-j]), ncol(othercount))
      ), drop = FALSE])
      #                                        sample(1: 3500           , min(500* 2                   ,   3500.   )

      for(nnn in seq_len(ncol(subc))) {
        ## tmp dim 1 * 500
